...
- The user-facing command and skill name are `xsearch`.
- The installed runtime is `~/.agents/tools/xai-xsearch/`.
...
- `node bin/xsearch.js --help` should print usage.
- `node bin/xsearch.js auth status` should run without throwing.
- `node scripts/xai-oauth.js --status` should run without throwing.
- `node scripts/xai-oauth.js --login` should reach the xAI device-code prompt.
- `node scripts/xai-search.js --help` should print usage.
- Live search requires a valid local OAuth token and an eligible xAI/X account.
